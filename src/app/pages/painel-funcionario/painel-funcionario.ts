...
export class PainelFuncionario {
  ...
  trancarMatricula(matriculaId?: number) {
    if (!matriculaId || !confirm('Deseja trancar esta matrícula?')) return;
    this.matriculaService.trancar(matriculaId).subscribe({
      next: () => {
        alert('Matrícula trancada!');
        this.carregarMatriculas();
      },
      error: err => alert('Erro ao trancar matrícula: ' + err.message)
    });
  }

  carregarMatriculas() {
    this.matriculaService.listar().subscribe({
      next: dados => this.matriculas = dados,
      error: err => console.error('Erro ao carregar matrículas:', err)
    });
  }
  ...
  adicionarAula(event: Event) {
    ...
    if (!this.novaAula.titulo.trim() || !this.novaAula.data.trim() ||
        !this.novaAula.hora.trim() || !this.novaAula.professorId) {
    ...
    this.aulaService.criar(this.novaAula).subscribe({
      next: () => {
        ...
        this.carregarAulas();
        this.novaAula = { titulo: '', data: '', hora: '', professorId: null };
      },
  ...
  prepararEdicao(aula: any) {
    ...
    this.novaAula = {
      ...
      data: aula.data,
      hora: aula.hora,
      professorId: aula.professorId || null
    };
  }

  salvarEdicao() {
    if (!this.aulaEditando || !this.novaAula.professorId) {
  ...
  adicionarProfessor(event: Event) {
    ...
    if (!prof.dataContratacao)
      prof.dataContratacao = new Date().toISOString().split('T')[0];

    this.professorService.criar(prof).subscribe({
      next: () => {
        alert('Professor cadastrado!');
        this.carregarProfessores();
        this.novoProfessor = { nome: '', cpf: '', dataContratacao: '' };
      },
  ...
  prepararEdicaoProfessor(prof: any) {
    ...
    this.novoProfessor = {
      nome: prof.nome,
      cpf: prof.cpf,
      dataContratacao: prof.dataContratacao
    };
  }

  salvarEdicaoProfessor() {
    if (!this.professorEditando || this.cpfInvalido) {
      alert("CPF inválido!");
      return;
    }
  ...
  adicionarAluno(event: Event) {
    ...
    if (!this.novoAluno.nome.trim()
      || !this.novoAluno.dataNascimento.trim()
      || !this.novoAluno.nomeResponsavel.trim()
      || !this.novoAluno.cpfResponsavel.trim()
      || !this.novoAluno.emailResponsavel.trim()
      || !this.novoAluno.telefoneResponsavel.trim()) {
  ...
  prepararEdicaoAluno(aluno: Aluno) {
